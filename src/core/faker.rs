use crate::core::protocol::{AnnounceRequest, AnnounceResponse, TrackerClient, TrackerError, TrackerEvent};
use crate::core::torrent::{ClientConfig, ClientType, TorrentInfo};
use crate::log_info;
use rand::Rng;
...
impl RatioFaker {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        // Update state
        *self.state.write().await = FakerState::Running;
    ...
    pub async fn stop(&mut self) -> Result<()> {
        ...
        // Update state
        *self.state.write().await = FakerState::Stopped;

        // CRITICAL: Also update the state in stats so frontend can detect the stop
        self.stats.write().await.state = FakerState::Stopped;
    ...
    pub async fn update(&mut self) -> Result<()> {
        ...
        if stats.left > 0 {
            ...
            if stats.left == 0 {
                ...
                stats = self.stats.write().await; // Re-acquire
            }
        }
    ...
    async fn on_completed(&mut self) -> Result<()> {
        ...
        // Update state
        *self.state.write().await = FakerState::Completed;
    ...
    pub async fn scrape(&self) -> Result<crate::core::protocol::ScrapeResponse> {
    ...
    pub async fn pause(&mut self) -> Result<()> {
        ...
        *self.state.write().await = FakerState::Paused;
        self.stats.write().await.state = FakerState::Paused;
        Ok(())
    }
    ...
    pub async fn resume(&mut self) -> Result<()> {
        ...
        *self.state.write().await = FakerState::Running;
        self.stats.write().await.state = FakerState::Running;
