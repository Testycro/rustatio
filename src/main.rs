...
use rustatio::core::validation;
use rustatio::{AppConfig, FakerConfig, FakerStats, RatioFaker, TorrentInfo};
...
#[tauri::command]
async fn create_instance(state: State<'_, AppState>) -> Result<u32, String> {
...
async fn list_instances(state: State<'_, AppState>) -> Result<Vec<InstanceInfo>, String> {
    ...
    for (id, instance) in fakers.iter() {
        ...
        instances.push(InstanceInfo {
            ...
            is_running: matches!(
                stats.state,
                rustatio::FakerState::Running | rustatio::FakerState::Completed
            ),
            is_paused: matches!(stats.state, rustatio::FakerState::Paused),
        });
    }
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            rustatio::core::logger::init_logger(app.handle().clone());
            Ok(())
        })
